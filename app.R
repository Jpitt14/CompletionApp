  ...
  fluidRow(
    column(6, 
           ...
           div(class = "table-container", 
               ...
               downloadButton("export_data", "Export", class = "shiny-btn")
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_team1, {
    new_data <- data.frame(
      ...
      Team = "COWBOYS",
      Condition1 = ifelse(selected_conditions$team1[1], "✔", "✖"),
      Condition2 = ifelse(selected_conditions$team1[2], "✔", "✖"),
      Condition3 = ifelse(selected_conditions$team1[3], "✔", "✖"),
    ...
    selected_conditions$team1 <- c(FALSE, FALSE, FALSE) # Reset conditions
    session$sendCustomMessage("resetBoxes", list(team = "team1")) # Reset UI boxes
  })
  ...
  observeEvent(input$submit_team2, {
    new_data <- data.frame(
      Order = nrow(results()) + 1,
      Team = "OPPOSITION",
      Condition1 = ifelse(selected_conditions$team2[1], "✔", "✖"),
      Condition2 = ifelse(selected_conditions$team2[2], "✔", "✖"),
      Condition3 = ifelse(selected_conditions$team2[3], "✔", "✖"),
      Result = as.character(sum(selected_conditions$team2)),
      ColorName = c("Black", "Red", "Yellow", "Green")[sum(selected_conditions$team2) + 1],
      stringsAsFactors = FALSE
    )
    results(rbind(new_data, results())) # Add the new row at the top
    selected_conditions$team2 <- c(FALSE, FALSE, FALSE) # Reset conditions
    session$sendCustomMessage("resetBoxes", list(team = "team2")) # Reset UI boxes
  })
  
  
  
  output$results_table <- renderReactable({
    req(nrow(results()) > 0)  # Ensure there's data to display
    ...
    reactable(
      ...
      columns = list(
        ...
        Condition1 = colDef(
          name = "Condition 1",
          align = "center",
          cell = function(value) div(style = "font-size: 18px;", value) # Render ✔/✖ larger
        ),
        Condition2 = colDef(
          name = "Condition 2",
          align = "center",
        ...
        ColorName = colDef(
          ...
          cell = function(value) {
            color <- switch(
              value,
              "Black" = "rgba(0, 0, 0, 0.2)",       # Faint black
              "Red" = "rgba(200, 0, 0, 0.3)",      # Faint red
              "Yellow" = "rgba(200, 200, 0, 0.3)", # Faint yellow
              "Green" = "rgba(0, 150, 0, 0.3)",    # Faint green
              "rgba(255, 255, 255, 0.1)"           # Default faint white
            )
            div(
              style = list(
